...
def download_images_from_csv(filepath, image_directory, api_key, has_categories=True):
    ...
    for _, row in df.iterrows():
        ...
        if has_categories:
            ...
            filename = '{}/{}.png'.format(os.path.join(dummy_category_path), name)
        else:
            dummy_category_path = os.path.join(image_directory, '0')
            if not os.path.isdir(dummy_category_path):
                os.mkdir(dummy_category_path)
            filename = '{}/{}.png'.format(os.path.join(dummy_category_path), name)
